...
const registerUser = asyncHandler(async (req, res) => {
  ...
  // Upload them to cloudnnary
  // Create user object - create entry in db
  // remove password and refresh token fient from response
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  // remove unwanted field before sending cookies
  const loggedInUser = await User.findById(user._id).select(
    "-password -refreshToken"
  );

  // set security option for cookie so that only server can modify that cookie
  const options = {
    httpOnly: true,
    secure: true,
  };
...
const updateAccountdetails = asyncHandler(async (req, res) => {
  ...
  const user = await User.findByIdAndUpdate(
    req.user?._id,
    {
      $set: {
        fullName,
        email: email, // different way to put value
      },
    },
    { new: true }
  ...
  return res
    .status(200)
    .json(new ApiResponce(200, user, "Account details update succesfully"));
});
...
const getWatchHistory = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(200, user[0].watchHistory, "Watch history fetch successfully");
});
export {
  registerUser,
  loginUser,
